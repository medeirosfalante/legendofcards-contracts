...
contract UpgradeController is Ownable {
    function migrate(address _oldAddress, address _newAddress)
        external
        onlyOwner
    {
        ...
        address[] memory _externalDependencies =
            _oldContract.getExternalDependencies();
        address[] memory _internalDependencies =
            _oldContract.getInternalDependencies();
        require(
        ...
        for (i = 0; i < _externalDependencies.length; i++) {
            ...
            _internalDependenciesOfExternal = _externalDependency
            ...
            _externalDependency.setInternalDependencies(
                _internalDependenciesOfExternal
            );
        }

        for (i = 0; i < _internalDependencies.length; i++) {
            _internalDependency = Upgradable(_internalDependencies[i]);
            _externalDependenciesOfInternal = _internalDependency
            ...
            _internalDependency.setExternalDependencies(
                _externalDependenciesOfInternal
            );
        }
